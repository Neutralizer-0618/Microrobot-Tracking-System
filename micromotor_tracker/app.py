...
def parse_mode_rectangles(canvas_result, frame_shape, scale_x: float, scale_y: float) -> Optional[List[Box]]:
...
def parse_mode_lines(canvas_result, scale_x: float, scale_y: float) -> Optional[List[Tuple[Tuple[float, float], Tuple[float, float]]]]:
    if canvas_result.json_data is None:
        return None
    lines = []
    for obj in canvas_result.json_data.get("objects", []):
...
canvas_result = st_canvas(
    ...
    key=f"main_canvas_{frame_index}_{annotation_mode}_{st.session_state['canvas_revision']}",
)

if annotation_mode == "positive":
    parsed_positive = parse_mode_rectangles(canvas_result, frame.shape, canvas_scale_x, canvas_scale_y)
    ...
    parsed_negative = parse_mode_rectangles(canvas_result, frame.shape, canvas_scale_x, canvas_scale_y)
...
with calibration_cols[1]:
    if st.button("Save calibration from frame"):
        parsed_lines = parse_mode_lines(canvas_result, canvas_scale_x, canvas_scale_y)
        if annotation_mode != "calibration" or not parsed_lines:
            st.error("Click 'Set fps and calibration', draw a line on the frame, then save.")
        else:
            line = parsed_lines[-1]
...
with roi_cols[0]:
    if st.button("Save ROI from frame"):
        parsed_boxes = parse_mode_rectangles(canvas_result, frame.shape, canvas_scale_x, canvas_scale_y)
        if annotation_mode != "roi" or not parsed_boxes:
            st.error("Click 'Select ROI', draw a blue rectangle on the frame, then save.")
        else:
...
st.subheader("Select positive and negative examples")
...
if st.button("Estimate object model from annotated boxes"):
    try:
